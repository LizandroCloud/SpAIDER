...
% uopt = [ uopt; ws(ks)*ones(length(tspan),1) ];
 ...
 if fix == 1   % Utilizando solver de passo fixo....
    if reg==0
        if est==1
               for ks = 1:ns
               [zs] = ode5( @(t,x)state(t,x,ws,ks,ts,reg), [ts(ks):0.05:ts(ks+1)], [z0]);
               z0 = zs(end,:)';
               topt = [ topt; tspan ];
               xopt = [ xopt; zs ];
               uopt = [ uopt; ws(ks)*ones(length(tspan),1) ];
...
%    % perfil nao regularizado... 
%    for i=1:(length(topt))
%         uopt(i) = u_reg0(topt(i),ts,ws,ks);
%    end 
%    
%     % impressao de perfil regularizado...  
%     for i=1:(length(topt))
%         u(i) = u_reg1(topt(i),ts,ws,ks);
%         u=[u];
%     end
% 
%    ureg1 = u;
...
%    fi=ts(1:ns);
% 
% % Mostrando o custo ótimo
% % disp(sprintf('Optimal cost for ns=%d: %d', ns));
% 
% Plotando resultados
 figure(1)
 plot(t,outp(:,3),'r')
 ...
 ylabel('Ca(t)')
 title(['ns=',int2str(ns)])
% % saveas(gcf,['x1(t)',int2str(ns),'stg.eps'], 'psc2')
% % 
% % figure(2+530)
% % plot(topt,xopt(:,2),'r')
% % xlabel('t')
% % ylabel('Cb(t)')
% % title(['ns=',int2str(ns)])
% % saveas(gcf,['x2(t)',int2str(ns),'stg.eps'], 'psc2')
% 
% figure(iu+630);
% plot(topt,u,'r')
% xlabel('t')
