...
#endif
{
 ...
 if (!(fout = freopen("dasslc_out.txt","w",stdout), fout)) mexErrMsgTxt("dasslc: could not open output file!");
 if (!(ferr = freopen("dasslc_err.txt","w",stderr), ferr)) {fclose(fout); mexErrMsgTxt("dasslc: could not open error file!");}
 ...
 if (output = dasslc (STEADY_STATE, &root, residuals, &t, tout, (*jac ? jacobian : NULL), NULL), output < 0)
   {CLOSEALL(); mexErrMsgTxt("dasslc: error in finding steady state!");}

 py = NEW(double, neq, "py");
 memcpy((void *)py, (void *)y, neq * sizeof(double));

 if (nlhs > 2)
   {
	pyp = NEW(double, neq, "pyp");
 ...
 switch (nlhs)
	{
	 ...
	 case 3: plhs[2] = mxCreateDoubleMatrix(1, neq, mxREAL);
			 ptr = (double *)mxGetPr(plhs[2]);
			 for (j = 0; j < neq; j++) ptr[j] = pyp[j]; 

	 default: plhs[1] = mxCreateDoubleMatrix(1, neq, mxREAL);
			  ptr = (double *)mxGetPr(plhs[1]);
			  for (j = 0; j < neq; j++) ptr[j] = py[j];
			  plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
		      memcpy((void *)mxGetPr(plhs[0]), (void *)&t, 1 * sizeof(double));
	}
