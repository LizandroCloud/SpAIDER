function [j4] = jacmat_prior(dfdu , e_var , par, c_var )
...
for i=1:length(dfdu(:,1))
 ...
 for i=1:length(dfdu1(:,1))
  ...
  for j=1:length(e_var)
   ...
   dfdu10(i,1:length(s2)) = str2sym(dfdu2(i,1:length(s2))); % new version
   s2=0;
 end 
...
for i=1:length(dfdu1(:,1))
    df(i,:)=jacobian(str2sym(dfdu2(i,:)),[e_var1 c_var1]); %jacobian2 is a modifyed
end
