...
if isempty(char(a_var))  % in case of only ode
    ...
    for i=1:length(a_var) % re-arranging (implicit derivation)
        ...
        a_eq  = regexprep((dx), s1, y(i)); % y() instead of y
        dx=a_eq; % storage
    end
    end
    [R2] = jacmat_prior( char(a_eq) , e_var, par, c_var ); % derivatives in relation to state var and control var
...
for i=1:length(e_var)  % calculate for each state variable
  ...
  %  R3 = regexprep(R3, 'x', 'f');  % change x for f to print later
	        for j=1:length(e_var) % path contraint 
            b=int2str(j);
