...
%                 fprintf(fid,'%c', 'c = spline([1:length(c)],c,1:300);');  
    ...
    fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'ko=numel(ceq); ki=1;');
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'jmax = length(f(1,:));  % states');
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'nk = length(stages)-1; % number of elements');
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'ne = length(f(:,1))/nk ; % lengh of the element'); 
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'for j=1:jmax'); 
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'for i=1:nk-1'); 
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'ceq(i+ko) = ( f(i*ne + 1,j) - f(i*ne,j) ) - 0;'); 
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'end'); 
     fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', ' ko = ko+i;'); 
     fprintf(fid,'%c\n', '');
     fprintf(fid,'%c', 'end'); 
      fprintf(fid,'%c\n', '');
    fprintf(fid,'%c', 'end');
