function [j4] = jacmat_states(dfdu , e_var , par, c_var )
...
for i=1:length(dfdu(:,1))
 ...
 for i=1:length(dfdu1(:,1))
  ...
  for j=1:length(e_var)
      if j==1
            ...
            s2 = regexprep(dfdu1(i,:), s1, [ ' y',int2str(j),' ']);
            dfdu2(i,1:length(s2)) = s2;
      else
            s1 = regexptranslate('escape',char(e_var(j)));
            s2 = regexprep(dfdu2(i,:), s1, [ ' y',int2str(j),' ']);
            dfdu2(i,1:length(s2)) = s2;
      end
  end
   dfdu10(i,1:length(s2)) = sym(str2sym(dfdu2(i,1:length(s2)))); % new version
   s2=0;
 end 
 ...
 for j=1:length(e_var)
            ...
            s1 = regexptranslate('escape',char(e_var(j)));
            s2 = regexprep(e_var(j), s1, [ ' y',int2str(j),' ']);
...
for i=1:length(dfdu1(:,1))
    df(i,:)=jacobian(str2sym(dfdu2(i,:)),[e_var1 ]); %NEW VERSION
end
...
%       df1 = regexprep(s, 'matrix', '');
  for j=1:length(e_var)
      ...
      if j==1
            s1 = regexptranslate('escape',char(e_var(j)));
            s2 = regexprep(df4, [ 'y',int2str(j)], s1);
            df5(i,1:length(s2)) = s2;
      else
            s1 = regexptranslate('escape',char(e_var(j)));
            s2 = regexprep(df5(i,1:length(s2)), [ 'y',int2str(j)], s1);
            df5(i,1:length(s2)) = s2;
      end
  end
  df6(i,:) = {df5(i,:)};
