function[is] =  plotp(t0,tf,is0,isF,nU,x0,u0,u_lb,u_ub,wname,tptr,sorh,scal,opC,adap,frozen,solver,plot,plot_state,tf_p,f_tol,f_tol2,t_ref,wx,tx,n_est,Fobj,time,time_ac,wfact,tunn,Pgr)
  % ************************************************************************
...
%  title(['Original wavelets coefficients for: ',int2str(ns_a(ic,:)),' stages']); 
 title(['Coeficientes wavelets originais para: ',int2str(ns_a(ic,:)),' estagios']); 
 else
%      title(['Original wavelets coefficients - control variable: ',int2str(ic),' for: ',int2str(ns_a(ic,:)),' stages'])
      title(['Coeficientes wavelets originais - variavel de controle: ',int2str(ic),' para: ',int2str(ns_a(ic,:)),' estagios'])
 end
% ylabel('Resolution levels')
ylabel('Niveis de resolucao')
xlabel('Estagios de discretizacao')
...
%  set(h,'Interpreter','none')
 axis([0 tf_p min(wopt(ic,:))*0.8  max(wopt(ic,:))*1.2 ])
  ...
  if nU==1
title(['Trajetoria de controle ',int2str(ns_a(ic,:)),' estagios']); 
 ylabel('u')
 xlabel('t')
 else
     title(['Trajetoria de controle: ',int2str(ic),' para: ',int2str(ns_a(ic,:)),' estagios'])
     ay = strcat('u-',int2str(ic));
     ylabel(ay)
     xlabel('t')
  end


 legend('estagios','pontos de discretizacao',1);
%  hold on;
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(ix2);
...
if nU==1
 title('Aproximacao wavelets'); 
 else
     title(['Aproximacao wavelets  - u: ',int2str(ic),' para: ',int2str(ns_a(ic,:)),' estagios'])
 end
ylabel('Coeficientes wavelets')
xlabel('Pontos')
...
if nU==1
 title('Valores de threshold');
 else
      title(['Valores de threshold  - u: ',int2str(ic),' para ',int2str(ns_a(ic,:)),' estagios'])
 end
ylabel('Coeficientes wavelets')
xlabel('Pontos')
...
if nU==1
 title('Coeficientes comprimidos') 
 else
      title(['Coeficientes comprimidos  - u: ',int2str(ic),' para: ',int2str(ns_a(ic,:)),' estagios'])
 end
ylabel('Nivel de resolucao')
xlabel('Estagios de discretizacao')
% colorbar
hold on;
...
%  legend('Stages','Discret points','Refinement region','',1);
 ...
 legend('Estagios','Pontos','Regiao de refinamento','Novos pontos',1);
%  set(h,'Interpreter','none')
 axis([0 tf_p min(wopt(ic,:))*0.8  max(wopt(ic,:))*1.2 ])
 if nU==1
 title(['Trajetoria refinada: ',int2str(ns_a(ic,:)),' estagios']);
 ylabel('Variavel de controle refinada')
 xlabel('t')
 else
%       title(['Refined dynamic control profile: ',int2str(ic),' for: ',int2str(ns_a(ic,:)),' stages'])
      title(['Trajetoria de controle refinada: ',int2str(ic),' para: ',int2str(ns_a(ic,:)),' estagios'])
%       ay = strcat('Refined control variable-',int2str(ic));
       ay = strcat('Trajetoria de controle refinada-',int2str(ic));
      ylabel(ay)
%        xlabel('Time')
       xlabel('t')
 end
...
%  set(h,'Interpreter','none')
 axis([0 tf_p min(wopt(ic,:))*0.8  max(wopt(ic,:))*1.2 ])
 if nU==1
 title(['Trajetoria de controle refinada para: ',int2str(ns_a(ic,:)),' estagios']);
  ylabel('Variavel de controle')
 xlabel('t')
 else
     title(['Trajetoria de controle refinada ',int2str(ic), ' para: ',int2str(ns_a(ic,:)),' estagios'])
     ay = strcat('Trajetoria de controle refinada-',int2str(ic));
