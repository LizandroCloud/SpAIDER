function[is] =  plotp(t0,tf,is0,isF,nU,x0,u0,u_lb,u_ub,wname,tptr,sorh,scal,opC,adap,frozen,solver,plot,plot_state,tf_p,f_tol,f_tol2,t_ref,wx,tx,n_est,Fobj,time)
  % ************************************************************************
...
conv =10;
 ...
 if opC==1  % case wavelets procedure
     if adap==1  % case adaptation on
         valK = 0;  % tunning parameter (valk: increase the correction of wavelet threshold)
...
for is = is0:isF  % number of stages is ns^is0 
...
% set(get(img,'parent'),'YtickLabel',[]); 
 ...
 if nU==1
 title('Original wavelets coefficients'); 
 else
     title(['Original wavelets coefficients - control variable:',int2str(ic)])
 end
...
%  hold on;
 stem(ts_x,wopt(ic,1:ns(ic)),'LineStyle','none','MarkerEdgeColor','k','MarkerFaceColor','k', 'MarkerSize',6); 
...
%  set(h,'Interpreter','none')
 axis([0 tf_p min(wopt(ic,:))*0.8  max(wopt(ic,:))*1.2 ])
  ...
  if nU==1
 title('Dynamic control profile'); 
 else
     title(['Dynamic control profile:',int2str(ic)])
 end
...
if nU==1
 title('Wavelets aproximation and details coefficients'); 
     ...
     title(['Wavelets aproximation and details coefficients  - control variable:',int2str(ic)])
 end
...
if nU==1
 title('Threshold values');
     ...
     title(['Threshold values  - control variable:',int2str(ic)])
 end
ylabel('Wavelets coefficients')
xlabel('Points')
...
if adap==0 % adap =0 (fixed)
 Nc = norm(C)/length(C);  % coefficient norm...
...
% [Cth thr] = mingcvsoft(C);
     ...
     [w01(ic,:),C,L,s,treshold] = wden(wopt(ic,:),tptr,sorh,scal,maxlev,wname); % denoising
     treshold=treshold(end:-1:1);
%      thr = thselect(wopt(ic,:),tptr)*ones(1,maxlev).*s2;  % threshold value...
     thrs(ic,1:length(treshold),is-is0)=treshold;  % threshold vector
end
...
if plot==1
 ...
 figure(ix4);
 subplot(2,1,1); 
 stem(ts_x,w01(ic,1:ns(ic)),'LineStyle','none','MarkerEdgeColor','k','MarkerFaceColor','k', 'MarkerSize',6); 
 legend('stages','discrete points',1);
%  grid on;
%  set(h,'Interpreter','none')
 ...
 if nU==1
 title('Thresholded dynamic control profile') 
 else
     title(['Thresholded dynamic control profile:',int2str(ic)])
 end
 ylabel('Control variable')
 xlabel('Time') 
...
% cfd = zeros(maxlev,length(wopt(ic,: ))); 
Pgrad = Blips(valK,w01(ic,:),C,L) % onlly selection of refined locations
...
if nU==1
 title('Thresholded wavelets coefficients') 
     ...
     title(['Thresholded wavelets coefficients  - control variable:',int2str(ic)])
 end
...
subplot(2,1,2) 
 ...
 title('Distribution of details smaller than threshold'); 
     ...
     title(['Distribution of details smaller than threshold  - control variable:',int2str(ic)])
 end
...
ts(ic,1:ns_ref+1)=ts_ref(1:ns_ref+1);
w0ns(ic,1:ns_ref)=w_ref(1:ns_ref);
ns(ic)=ns_ref;  
...
%  legend('Stages','Discret points','Refinement region','',1);
 hold on;
 stem(ts_x,w0ns(ic,1:ns(ic)),'LineStyle','none','Marker','x','MarkerEdgeColor','k','MarkerFaceColor','k', 'MarkerSize',4); 
 legend('Stages','Discret points','Refinement region','New points',1);
%  set(h,'Interpreter','none')
 axis([0 tf_p min(wopt(ic,:))*0.8  max(wopt(ic,:))*1.2 ])
 if nU==1
 title('Refined dynamic control profile');
 else
     title(['Refined dynamic control profile:',int2str(ic)])
 end
...
%  stairs(tspan,u_p,'k');
% %  hold on;
 stem(ts_a(ic,2:ns_a(ic)+1),wopt(ic,1:ns_a(ic)),'LineStyle','none','MarkerEdgeColor','k','MarkerFaceColor','k', 'MarkerSize',4); 
...
% wopt=[];
 ...
 if is>is0
     conv(is-2)=abs((abs(Fobj(is))-abs(Fobj(is-1)))*0.1*abs((Fobj(is)))^-1) ;
 else
     conv(is-2)=10;
 end
 
 if is>is0+1
     convv(is-2)=abs((abs(conv(is-3))-abs(conv(is-4)))*abs((conv(is-4)))^-1) ;
 else
     convv(is-2)=10;
 end
