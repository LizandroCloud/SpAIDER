function [ f, t_ult, t_out, y, ti, df ] = function_unisim( x0, ns, ts, ws, ne, n_s,n_c,w_froz,check,ns_a,frozen,is,is0,w0,ts_frozen,solver,optODE,typ,reg,est,optSPAIDER )
 
% global reg;
% global est;
global DEBUG;  % for dassl solver...
% global t_ult;
% hy = hyconnect;   %Conectar Hysys ao activeX controller.
% hysys = hy.SimulationCases.Open([cd,'\gss.usc']); %Abrindo a simulação
% hysys.Visible = true; %Visibilidade
global hy;
global hysys;
%% Parameters
DEBUG  = 1; 
...
%                  tic
                   [~,~] = model_unisim_o(ti,dt,u)
                   [tspan,zs] = model_unisim(ti,dt,u)
...
%                  [tspan,hh,zs] = sim('emso',[ti(1) :( (ti(ks+1)-ti(ks))/10): ti(ks+1)],[],[ti(ks+1)' u ]);
                   [~,~] = model_unisim_o(ti(ks+1),1,u,ts,ks,hy,hysys)
                   [tspan,zs] = model_unisim(ti(ks+1),10,u,ts,ks,hy,hysys);
