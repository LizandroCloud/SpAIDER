function [ f, t_ult, t_out, y, ti, df ] = function_mbo( x0, ns, ts, ws, ne, n_s,n_c,w_froz,check,ns_a,frozen,is,is0,w0,ts_frozen,solver,optODE,typ,reg,est,optSPAIDER )
...
%             span = (0 :ts(ns+1)/d_ode:ts(ns+1));
                ...
                wsx=ws; tex=ts; ksx=ks; nex=ns;
                
                UX=yi;
%               z0(210)=u(i);
                A=1;
               [tspan,zs] = ode15i('daesys_ss',[ti(1):(ti(end)-ti(1))/50:ti(end)],z0,ydot0);
