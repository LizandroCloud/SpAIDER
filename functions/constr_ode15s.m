...
if nargout == 2   % numerical jacobian
	[f, t_ult,t_out,yout,n_elem] = function_ode15s( x0, ns, ts, ws, ne,n_s,n_c, w_froz,check,ns_a,frozen,is,is0,w0,ts_frozen,solver,optODE,typ,reg,est,optSPAIDER);
	[ceq c] = cr1(f, t_ult,t_out,ws,yout,n_elem); % function for calculate constraints
else              % analytical jacobian
	[f,t_ult,t_out,yout,n_elem,df] = function_ode15s( x0, ns, ts, ws, ne,n_s,n_c, w_froz,check,ns_a,frozen,is,is0,w0,ts_frozen,solver,optODE,typ,reg,est,optSPAIDER);
